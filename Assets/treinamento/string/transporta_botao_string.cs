...
public class transporta_botao_string : MonoBehaviour {

	public GameObject [] condicao = new GameObject[5];
	public GameObject [] opcao = new GameObject[9];
	public bool [] b_condicao = new bool[5];
	...
	public void apertou(GameObject botao){  
		...
		if(b_opcao[i]==-1){
			if(b_condicao[0]==false){
				...
				b_condicao[3]=true;
				b_opcao[i]=3;
			}else if(b_condicao[4]==false){
				botao.transform.position = condicao[4].transform.position;
				b_condicao[4]=true;
				b_opcao[i]=4;
			}
		}else{
